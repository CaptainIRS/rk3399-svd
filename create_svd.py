...
device_maps = {
    ...
    "MMU": [
        ...
        {"name": "MMU_VOPB", "base": "0xff900300", "description": "Registers of Memory Management Unit (MMU) for Video Output Processor (Big) (VOPB)"},
        {"name": "MMU_VOPL", "base": "0xff8f0300", "description": "Registers of Memory Management Unit (MMU) for Video Output Processor (Little) (VOPL)"},
...
peripherals = {p: x for p, x in sorted(peripherals.items(), key=lambda x: x[0])}
...
peripherals = {p: x for p, x in sorted(peripherals.items(), key=lambda x: ordered.index(x[0]))}
    ...
    if 'derivedFrom' in peripherals[peripheral]:
        ...
        for r, register in enumerate(peripherals[peripheral]['registers']):
            ...
            for i, field in enumerate(register['fields']):
                ...
                field_exclusions = ['HCON', 'SCALEDOWN', 'ULSTCHNGREQ', 'USBLNKST', 'SCL_ST', 'SDA_ST', 'RESISTOR_CTRL', 'SDCLKFREQSEL', 'DP_TX_VERSION']
                ...
                if len(enums) >= 2 and '......' not in field['description'] and field['name'] not in field_exclusions and register['name'] not in register_exclusions:
                    ...
                    if len(enum_decs) != len(set(enum_decs)):
                        print(register['name'], field['name'], enum_names)
